...
interface ComplianceRequirement {
  ...
  dueDate?: string;
  category: 'regulatory' | 'reporting' | 'kyc' | 'aml';
}

function ComplianceStatusWidget({ companyId }: { companyId: string }) {
  // Mock compliance data - in real implementation, fetch from API
  const [complianceData, setComplianceData] = useState<{
    ...
    score: 87,
    requirements: [
      { id: '1', name: 'AIFM-rapportering Q4', status: 'pending', dueDate: '2026-01-31', category: 'reporting' },
      { id: '2', name: 'KYC-uppdatering investerare', status: 'completed', category: 'kyc' },
      { id: '3', name: 'AML-screening', status: 'completed', category: 'aml' },
      { id: '4', name: 'Riskkartläggning 2026', status: 'pending', dueDate: '2026-03-15', category: 'regulatory' },
      { id: '5', name: 'SFDR Artikel 8 rapportering', status: 'overdue', dueDate: '2026-01-05', category: 'reporting' },
    ],
    recentDocuments: [
      { id: '1', name: 'AIFM-manual 2025', date: '2025-12-15' },
      { id: '2', name: 'KYC-policy uppdaterad', date: '2025-11-20' },
      { id: '3', name: 'AML-riskbedömning', date: '2025-10-30' },
    ],
  });

  const completedCount = complianceData.requirements.filter(r => r.status === 'completed').length;
  const pendingCount = complianceData.requirements.filter(r => r.status === 'pending').length;
  const overdueCount = complianceData.requirements.filter(r => r.status === 'overdue').length;
    ...
    <div className="space-y-6">
      ...
      <div className="p-6">
        <div className="flex flex-col lg:flex-row lg:items-center gap-6">
          {/* Score Circle */}
          <div className="relative w-28 h-28 flex-shrink-0 mx-auto lg:mx-0">
            <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
              ...
              <circle
                ...
                r="42"
                fill="none"
                stroke={complianceData.score >= 80 ? '#22c55e' : complianceData.score >= 60 ? '#f59e0b' : '#ef4444'}
                strokeWidth="8"
                strokeDasharray={`${(complianceData.score / 100) * 264} 264`}
                strokeLinecap="round"
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                <p className="text-2xl font-bold text-aifm-charcoal">{complianceData.score}%</p>
                <p className="text-[10px] text-aifm-charcoal/50 uppercase tracking-wider">Score</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 grid grid-cols-3 gap-4">
            <div className="text-center lg:text-left p-3 bg-emerald-50 rounded-xl">
              <div className="flex items-center justify-center lg:justify-start gap-2 mb-1">
                <CheckCircle2 className="w-4 h-4 text-emerald-600" />
                <span className="text-2xl font-bold text-emerald-600">{completedCount}</span>
              </div>
              <p className="text-xs text-emerald-600/70 font-medium">Klara</p>
            </div>
            <div className="text-center lg:text-left p-3 bg-amber-50 rounded-xl">
              <div className="flex items-center justify-center lg:justify-start gap-2 mb-1">
                <Clock className="w-4 h-4 text-amber-600" />
                <span className="text-2xl font-bold text-amber-600">{pendingCount}</span>
              </div>
              <p className="text-xs text-amber-600/70 font-medium">Pågående</p>
            </div>
            <div className="text-center lg:text-left p-3 bg-red-50 rounded-xl">
              <div className="flex items-center justify-center lg:justify-start gap-2 mb-1">
                <AlertTriangle className="w-4 h-4 text-red-600" />
                <span className="text-2xl font-bold text-red-600">{overdueCount}</span>
              </div>
              <p className="text-xs text-red-600/70 font-medium">Försenade</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-gray-100">
        <div className="p-4 space-y-2">
          {complianceData.requirements.slice(0, 4).map((req) => (
            <div 
              key={req.id}
              className={`p-3 rounded-xl flex items-center justify-between gap-3 transition-all ${
                req.status === 'overdue' ? 'bg-red-50' : 'bg-gray-50 hover:bg-gray-100/50'
              }`}
            >
              <div className="flex items-center gap-3 min-w-0">
                <div className={`w-2 h-2 rounded-full ${getStatusColor(req.status)}`} />
                <div className="min-w-0">
                  <p className="text-sm font-medium text-aifm-charcoal truncate">{req.name}</p>
                  {req.dueDate && (
                    <p className="text-xs text-aifm-charcoal/40">
                      Deadline: {new Date(req.dueDate).toLocaleDateString('sv-SE')}
                    </p>
                  )}
                </div>
              </div>
      ...
      <div className="border-t border-gray-100 p-4">
        <p className="text-xs uppercase tracking-wider font-semibold text-aifm-charcoal/40 mb-3">
          Senaste compliance-dokument
        </p>
        <div className="space-y-2">
          {complianceData.recentDocuments.map((doc) => (
            <Link
              key={doc.id}
              href={`/compliance/documents?doc=${doc.id}`}
              className="flex items-center justify-between p-3 rounded-xl bg-gray-50 hover:bg-aifm-gold/5 group transition-colors"
            >
              <div className="flex items-center gap-3">
                <FileCheck className="w-4 h-4 text-aifm-charcoal/40 group-hover:text-aifm-gold transition-colors" />
                <span className="text-sm font-medium text-aifm-charcoal group-hover:text-aifm-gold transition-colors">{doc.name}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-xs text-aifm-charcoal/40">{doc.date}</span>
                <ExternalLink className="w-3.5 h-3.5 text-aifm-charcoal/30 group-hover:text-aifm-gold transition-colors" />
              </div>
            </Link>
          ))}
        </div>
      ...
      {/* Go to Compliance Manager */}
      <div className="border-t border-gray-100 p-4">
        <Link
          href={`/?view=compliance&companyId=${companyId}`}
          className="flex items-center justify-center gap-2 w-full px-4 py-3 bg-aifm-charcoal text-white rounded-xl hover:bg-aifm-charcoal/90 transition-colors text-sm font-medium"
        >
  ...
  return (
    <>
      ...
      <div className="space-y-6">
        ...
        {/* Compliance Status - Always visible first */}
        <CollapsibleSection 
          title="Compliance Status" 
          icon={Shield} 
          defaultOpen={true}
          linkHref="/?view=compliance"
          linkText="Compliance Manager"
        >
          <ComplianceStatusWidget companyId={selectedCompany.id} />
        </CollapsibleSection>
