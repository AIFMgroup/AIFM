...
function TasksContent() {
  ...
  const getDueDateInfo = (dueDate?: string) => {
    ...
    if (isToday(date)) return { label: 'Idag', className: 'text-amber-600 bg-amber-50' };
    if (isTomorrow(date)) return { label: 'Imorgon', className: 'text-blue-600 bg-blue-50' };
    if (isPast(date)) return { label: 'Försenad', className: 'text-red-600 bg-red-50' };
    return { label: format(date, 'd MMM', { locale: sv }), className: 'text-gray-500 bg-gray-100' };
  };
    ...
    <CrmLayout>
      <div className="px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Uppgifter</h1>
            <div className="flex flex-wrap items-center gap-2 sm:gap-4 mt-1 text-xs sm:text-sm">
              ...
              {stats.today > 0 && (
                <span className="text-amber-600 font-medium">{stats.today} idag</span>
              )}
              <span className="text-gray-500">{stats.upcoming} öppna</span>
            </div>
          </div>
          <button
            onClick={() => {
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 mb-6">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#c0a280] focus:border-transparent text-sm"
            />
          </div>
          <div className="flex gap-2 sm:gap-4">
            <select
              ...
              className="flex-1 sm:flex-none px-3 sm:px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-[#c0a280] focus:border-transparent bg-white"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className={`${showForm ? 'lg:col-span-2' : 'lg:col-span-3'}`}>
            {loading ? (
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-1">Inga uppgifter</h3>
          ...
          {showForm && (
            <div className="fixed inset-0 z-50 md:static md:z-auto lg:col-span-1">
              ...
              <div className="absolute inset-x-0 bottom-0 top-16 md:static bg-white rounded-t-2xl md:rounded-xl border border-gray-200 p-4 sm:p-6 overflow-y-auto md:sticky md:top-4">
                <TaskForm
                  task={selectedTask}
                  contacts={contacts}
                  companies={companies}
