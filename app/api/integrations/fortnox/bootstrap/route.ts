...
/**
 * GET /api/integrations/fortnox/bootstrap?companyId=xxx
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('[Fortnox] Integrations bootstrap status error:', error);
    return NextResponse.json({ error: 'Failed to get bootstrap status' }, { status: 500 });
  }
}

/**
 * POST /api/integrations/fortnox/bootstrap?companyId=xxx
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[Fortnox] Integrations bootstrap start error:', error);
