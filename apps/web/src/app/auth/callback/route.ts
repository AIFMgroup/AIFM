...
cookieHeader: ${cookieHeader || "none"}</pre>
  ...
  const sanitizeReturnTo = (value: string | undefined) => {
    // Default to dashboard, not root
    if (!value || value === "/") return "/dashboard";
    try {
      // If it's an absolute URL, only allow our CloudFront host; otherwise, default to dashboard.
      const url = new URL(value, "https://d31zvrvfawczta.cloudfront.net");
      const allowedHost = "d31zvrvfawczta.cloudfront.net";
      if (url.hostname !== allowedHost) return "/dashboard";
      const path = url.pathname + (url.search || "") + (url.hash || "");
      // If somehow ended up at root or sign-in, go to dashboard
      return path === "/" || path.startsWith("/sign-in") ? "/dashboard" : path;
    } catch {
